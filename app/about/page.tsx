...
function CountUp({ to }: { to: number; suffix?: string }) {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true });
    
    // Simple spring animation for number could be implemented here or just a CSS transition
    // For simplicity, we'll just show the number when in view for now, or use a library
    // But since we want to keep it simple without extra libs:
    return (
        <span ref={ref} className="tabular-nums">
             {/* Simulating count up would require state/effect, for now static but revealed */}
             {isInView ? to : 0}{to === 1000000 ? "M+" : to === 100 ? "%" : "+"}
        </span>
    );
}
        ...
        <div className="overflow-hidden">
             ...
             <div ref={heroRef} className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
                <motion.div style={{ y: bgY }} className="absolute inset-0 z-0 will-change-transform">
                    <Image
                        src="/5.jpg" // Using an existing image
                        alt="About Heureuse"
                        fill
